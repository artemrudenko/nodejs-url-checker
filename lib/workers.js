...
workers.allertUserToStatusChange = function(newCheckData) {
  ...
  helpers.sendTwilioSms(newCheckData.userPhone, message, function(err){
    if (!err) {
      console.log('Success: User was allerted to a status change in their check, via sms: '+ message);
    } else {
      console.log('Error: could not send sms alert to user who has a status change in their check!');
    }
  });
};
